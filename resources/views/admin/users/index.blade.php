            ...
            @forelse($users as $user)
                <tr>
                    ...
                    <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                        ...
                        @if($user->id !== Auth::id())
                            <form class="inline-block" action="{{ route('admin.users.destroy', $user) }}" method="POST" onsubmit="return confirm('Are you sure you want to delete this user?');">
                                @csrf
                                @method('DELETE')
            ...
            @empty
                <tr>
                    <td colspan="5" class="px-6 py-4 whitespace-nowrap text-sm text-gray-500 text-center">
                        No users found. <a href="{{ route('admin.users.create') }}" class="text-indigo-600 hover:text-indigo-900">Create one</a>.
                    </td>
                </tr>
            @endforelse
        </tbody>
